...
directory = '/home/awickert/Dropbox/Papers/InProgress/ChannelWidth/dakota_minnesota/logvars/out/run.'+run_number+'/'

params, results = di.read_parameters_file(parameters_file=directory+'params.in', results_file=directory+'results.out')

################
# Observations #
################
obs = pd.read_csv('DevonLibby_Table3-3_Reach08.csv')
...
# Predictions #
###############
rw = ottar.RiverWidth.from_yaml('config-dakota-params.yaml')
...
for _yr in comp['Year']:
    try:
        comp['Width modeled [m]'][comp['Year'] == _yr] = \
            modeled_width_by_year[_yr]
    except:
        comp['Width modeled [m]'][comp['Year'] == _yr] = np.nan
